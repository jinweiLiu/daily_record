...
<img src="C:\Users\jwliu\AppData\Roaming\Typora\typora-user-images\image-20210929101550720.png" alt="image-20210929101550720" style="zoom:80%;" />
...
class LRUCache {
    ...
    Map<Integer,DeNode> map = new HashMap<>();
    int size;
    int capacity;
    private DeNode head, tail;
    ...
    public void removeNode(DeNode node){
        node.pre.next = node.next;
        node.next.pre = node.pre;
    }

    public void addToHead(DeNode node){
        node.next = head.next;
        node.pre = head;
        head.next.pre = node;
        head.next = node;
    ...
    public void moveToHead(DeNode node){
        removeNode(node);
        addToHead(node);
    }

    public DeNode reomveTail(){
        DeNode res = tail.pre;
        removeNode(res);
        return res;
    }
