...
class ShowGradCam:
    ...
    def show_on_img(self,input_img):
        ...
        print(fmap.shape, grads_val.shape)
        cam = self.gen_cam(fmap[0], grads_val[0])
        cam = cv2.resize(cam, (28,28))
        heatmap = cv2.applyColorMap(np.uint8(255 * cam), cv2.COLORMAP_JET)/255.
        cam = heatmap * 0.4 + np.float32(input_img/255.) #0.4控制显示的透明度
        cam = cam / np.max(cam)*255
        cv2.imwrite('pics/heat.jpg',cam)
        print('save gradcam result in grad_feature.jpg')
```

```python
if __name__ == '__main__':
    path_img = os.path.join("cat.jpg")
    path_net = 'net.pth' #对应网络参数

    # 图片读取；网络加载
    img = cv2.imread(path_img, 1)  # H*W*C
    img_input = img_preprocess(img) #图片送入网络前的预处理
    net = Net() #定义模型

    gradCam = ShowGradCam(net.conv2) #定义要生成热力图的具体层
    ...
    #来源 https://www.jianshu.com/p/7f54d8cadeca
```
